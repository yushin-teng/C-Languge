...
template <typename T> class Stack
{
  ...
  private:
    ...
    ostream& writeNodes(ostream& out, Node* pNode) const;  // Write Node objects to a stream
    Node* readNodes(istream& in);                          // Read Node objects from stream
};
...
template <typename T> ostream& Stack<T>::writeNodes(ostream& out, Node* pNode) const
{
  while(pNode)
  {
    out << ' ' << *(pNode->pItem);
...
template <typename T> Stack<T>::Node* Stack<T>::readNodes(istream& in)
{
